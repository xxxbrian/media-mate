...
function normalizeRepoFromUrl(repoUrl?: string): string {
  if (!repoUrl) return '';
  return repoUrl
...
function resolveUpdateRepo(): string {
  if (process.env.NEXT_PUBLIC_UPDATE_REPO) {
    return process.env.NEXT_PUBLIC_UPDATE_REPO;
  }
  const inferredFromUrl = normalizeRepoFromUrl(process.env.NEXT_PUBLIC_REPO_URL);
  if (inferredFromUrl) return inferredFromUrl;
  const owner = process.env.NEXT_PUBLIC_REPO_OWNER;
  const slug = process.env.NEXT_PUBLIC_REPO_SLUG;
  if (owner && slug) {
    return `${owner}/${slug}`;
  }
  return '';
...
export const UPDATE_REPO = resolveUpdateRepo();
export const UPDATE_REPO_URL = UPDATE_REPO
  ? `https://github.com/${UPDATE_REPO}`
  : '';
...
const VERSION_SOURCE_URLS = UPDATE_REPO
  ? [
...
const PACKAGE_SOURCE_URLS = UPDATE_REPO
  ? [
  ...
  try {
    ...
    if (hasUpdate) {
      ...
      const remoteVersion: RemoteVersionInfo = {
        ...
        downloadUrl: UPDATE_REPO_URL
          ? `${UPDATE_REPO_URL}/releases`
          : undefined,
      };
